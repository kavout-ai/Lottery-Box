...
// 新增：自转控制变量
var sphereRotating = false;
var sphereRotationSpeed = 0.01;
...
function init() {
  ...
  // 新增：将所有 objects 添加到 sphereGroup
  for (var i = 0; i < objects.length; i++) {
    sphereGroup.add(objects[i]);
  }
  // 新增：将 sphereGroup 添加到 scene
...
function animate() {
  ...
  TWEEN.update();
  // sphere自转
  if (sphereRotating && sphereGroup) {
    sphereGroup.rotation.y += sphereRotationSpeed;
    sphereGroup.rotation.x += sphereRotationSpeed * 0.5;
    render(); // 需要实时渲染
  }
  // controls.update();
}
